...
- **Chromium: extension icon replaced SVG with PNG.** Chrome does not support SVG extension icons. The Chromium build now bundles PNG icons at 16×16, 32×32, 48×48, and 128×128 generated by `dev/render_icon_pillow.py`. The SVG is excluded from the Chromium zip entirely.
...
- First release to the Chrome Web Store. Firefox remains at 1.3.0 (no Firefox-specific changes in this version).
