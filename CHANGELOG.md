...
- **Minimum desktop Firefox version restored to 126.** Version 1.1 raised the floor to 140 on the assumption that the `data_collection_permissions` manifest key required it, but older Firefox simply ignores unknown manifest keys — no minimum version is needed for the key to be valid. Since this extension declares `required: ["none"]` there is no consent flow to provide on older versions anyway, so the 140 floor was costing the 128 ESR audience for no benefit. The Android minimum remains at 140 (Android genuinely requires it for general-purpose extension support).
